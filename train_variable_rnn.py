...
diag_list = [diag_code for patient_list in train_seqs for adm_list in patient_list for diag_code in adm_list]
num_features = len(set(diag_list))
print(num_features)
...
for epoch in range(NUM_EPOCHS):
	...
	if is_best:
		...
		print('SAVING RNN MODEL: ', PATH_OUTPUT+"MyVariableRNN.pth")
		# print(model)
...
def predict_mortality(model, device, data_loader):
	...
	with torch.no_grad():
		for i, (input, target) in enumerate(data_loader):
			...
			y_pred = y_pred.detach().to('cpu').numpy()[1]
			results.append(y_pred)
	return results

print('About to predict mortality')
